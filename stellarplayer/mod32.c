...
static void loadHeader()
{
	...
	if(!strncmp((const char*)temp + 1, "CHN", 3))
		uMod.Mod.numberOfChannels = temp[0] - '0';
	else if(!strncmp((const char*)temp + 2, "CH", 2))
...
static void processRow()
{
	...
	for(channel = 0; channel < uMod.Mod.numberOfChannels; channel++) {
		...
		if(sampleNumber) {
			...
			if(!(effectParameter == 0xE && effectParameterX == MOD32_NOTEDELAY))
		...
		if(note != NONOTE_MOD) {
			...
			if(effectNumber != MOD32_TONEPORTAMENTO && effectNumber != MOD32_PORTAMENTOVOLUMESLIDE)
		...
		case MOD32_PORTAMENTOVOLUMESLIDE:
			uPlayer.Mod_player.portamentoNote[channel] = uPlayer.Mod_player.amigaPeriod[channel];
			note = NONOTE_MOD;
			break;
		...
		case MOD32_SETCHANNELPANNING:
			Mixer.channelPanning[channel] = effectParameter >> 1;
			break;

		case MOD32_SETSAMPLEOFFSET:
		...
		case 0xE:
			...
			case MOD32_FINEVOLUMESLIDEDOWN:
				uPlayer.Mod_player.volume[channel] -= effectParameterY;
		...
		if(note != NONOTE_MOD || uPlayer.Mod_player.lastAmigaPeriod[channel] &&
				effectNumber != MOD32_VIBRATO && effectNumber != MOD32_VIBRATOVOLUMESLIDE &&
				!(effectNumber == 0xE && effectParameterX == MOD32_NOTEDELAY))
			Mixer.channelFrequency[channel] = uPlayer.Mod_player.amiga / uPlayer.Mod_player.lastAmigaPeriod[channel];
